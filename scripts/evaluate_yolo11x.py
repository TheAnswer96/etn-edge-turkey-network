"""
Evaluate yolo11x.pt (YOLO11X detection, behavior classes) on the test set.

yolo11x.pt is trained on turkey behavior classes (multiple class IDs).
All predicted classes are collapsed to class 0 so that detection performance
can be measured against GT class 0 (turkey body), independent of behavior.

Metrics: Precision, Recall, AP50 (VOC 2010+ all-points interpolation).

Usage:
    python scripts/evaluate_yolo11x.py
    python scripts/evaluate_yolo11x.py --conf 0.25 --iou 0.5
"""
...
def evaluate(
    ...
    print(f"  Note   : all predicted classes collapsed → class 0 (turkey)")
    ...
    for idx, img_path in enumerate(image_paths, 1):
        ...
        # cv2.imread avoids ultralytics path-parser issues with spaces in filenames
        ...
        # GT for this image (class 0 body only)
        label_path = test_labels / (img_path.stem + ".txt")
        gt_xyxy    = load_gt_xyxy(label_path)
        n_gt       = len(gt_xyxy)
        total_gt  += n_gt

        # All predicted boxes regardless of behavior class → treated as class 0
        ...
        for result in results:
            if result.boxes is None or len(result.boxes) == 0:
                continue
            boxes = result.boxes.xyxyn.cpu().numpy()  # [P,4] normalised xyxy
...
def _args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate yolo11x.pt (behavior model) on turkey test set."
    )
